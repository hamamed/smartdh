...
// Digital Asset Links — verifies the Android TWA app owns this domain (removes the URL bar).
// The file is generated by the build-apk workflow; served here because express.static skips dotfiles.
app.get('/.well-known/assetlinks.json', (req, res) => {
  const f = path.join(__dirname, 'public', '.well-known', 'assetlinks.json');
  if (fs.existsSync(f)) { res.type('application/json'); return res.sendFile(f); }
...
app.get('/download', (req, res) => {
  // Show the APK button only once the CI build has committed the file into the repo.
  const apkExists = fs.existsSync(path.join(__dirname, 'public', 'downloads', 'kanzup.apk'));
  const apkUrl = process.env.APK_URL || (apkExists ? '/downloads/kanzup.apk' : '');
  res.render('download', { title: req.t('dl_title'), apkUrl });
});
